 ...
 * Each todo carries the exact DiagramCommand that will execute it so the
 * apply path routes through the existing executor unchanged.
 */
export interface TranscriptTodo {
  /** Stable id generated by the AI (e.g. "t1") */
  id: string;
  /** Human-readable action item extracted from the transcript */
  text: string;
  /** Verbatim transcript snippet (≤ 200 chars) that supports this todo */
  quote: string;
  /** 0–1 confidence: 0.9+ explicit decision, 0.6–0.8 implied, <0.6 ambiguous */
  confidence: number;
  /** The exact command to run when this todo is accepted */
  command: DiagramCommand;
}

/**
 * Full result of the transcript analysis LLM call.
 */
export interface TranscriptAnalysis {
  /** 2–3 sentence AI summary of what was discussed and decided */
