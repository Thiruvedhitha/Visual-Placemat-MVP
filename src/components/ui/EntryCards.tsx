...
const aiCard = {
  id: "ai-prompt",
  title: "Start with AI Prompt / Available Templates",
  subtitle: "Pick a template or start blank with AI",
  icon: (
  ...
  return (
    <>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5">
        ...
        {cards.map((card) => {
          ...
          return (
            <Link
              ...
              `}
            >
              <div className={`flex h-14 w-14 items-center justify-center rounded-xl bg-brand-50 transition-transform duration-200 ${isHovered ? "scale-110" : ""}`}>
                {card.icon}
              </div>
        ...
        {(() => {
          const isHovered = hoveredId === aiCard.id;
          return (
            <button
              type="button"
              onClick={() => setTemplateOpen(true)}
              onMouseEnter={() => setHoveredId(aiCard.id)}
              onMouseLeave={() => setHoveredId(null)}
              className={`
                group relative flex flex-col items-center gap-3 rounded-2xl border-2 bg-white px-5 py-8 shadow-md
              ...
              `}
            >
              <div className={`flex h-14 w-14 items-center justify-center rounded-xl bg-slate-50 transition-transform duration-200 ${isHovered ? "scale-110" : ""}`}>
                {aiCard.icon}
              </div>
              <div className="text-center">
                <span className="block text-sm font-semibold text-slate-800 group-hover:text-brand-700">
